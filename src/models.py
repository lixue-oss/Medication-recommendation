...
class SafeDrugModel(nn.Module):
    def __init__(
        ...
        # pre-embedding
        #诊断嵌入和手术嵌入（diagnosis embedding和procedure embedding）
        ...
        # )
        #应该是上面的代码，让我改成了LSTM,又加了一个双向,bidirectional=True
        #不反向传播的时候，01等于torch.Size([1, 1, 64])
         #注意力机制模块+双向LSTM
        print('88这是ATT+BILSTM的训练结果：')
        self.encoders = nn.ModuleList(
            [nn.LSTM(emb_dim, emb_dim, batch_first=True,bidirectional=True) for _ in range(2)]
        )
        ...
        #LSTM和GRU双向的时候，需要把下面的2改成4；如果不是双向的，应该是2
    ...
    def forward(self, input):
        ...
        # print(o1.shape)
        ## 我自己写的，注意力层
        attention_weights_o1 = torch.softmax(self.attention(o1), dim=1)
        # print('attention_weights_o1的形状是什么')
        # print(attention_weights_o1.shape)
        attention_output_o1 = torch.sum(attention_weights_o1 * o1, dim=1)
        # print('attention_output_o1的形状是什么')
        # print(attention_output_o1.shape)

        o2,h2 = self.encoders[1](i2_seq)
        o2 = self.fc(o2)
        ## 我自己写的，注意力层
        attention_weights_o2 = torch.softmax(self.attention(o2), dim=1)
        attention_output_o2 = torch.sum(attention_weights_o2 * o2, dim=1)
        # print('attention_output_o2的形状是什么')
        # print(attention_output_o2.shape)
        #我自己写的，
        patient_representations = torch.cat([attention_output_o1, attention_output_o2], dim=-1) # (seq, dim*2)
        # print('patient_representations 是什么？？？？？？？？')
        # print(patient_representations)
        ...
        print('MPNN_att 是什么？？？？？？？？？')
        print(MPNN_att.shape)
        # local embedding
        #bipartite_emb是local embedding嵌入结果
        bipartite_emb = self.bipartite_output(
            F.sigmoid(self.bipartite_transform(query)), self.tensor_ddi_mask_H.t()
        )
        print('bipartite_emb是什么？？？？？？？')
        print(bipartite_emb.shape)
        #两个最终结果相乘一下
...
class TransGAHNet(nn.Module):
    def __init__(
        ...
        super(TransGAHNet, self).__init__()
        self.nhead = 8
        K = len(vocab_size)
        self.K = K
        self.vocab_size = vocab_size
        ...
        self.num_layers = 6
        self.encoders = nn.ModuleList(
            [nn.TransformerEncoderLayer(emb_dim, self.nhead,self.num_layers, batch_first=True,dropout=0.2) for _ in range(K - 1)]
        )
        self.bipartite_transform = nn.Sequential(
            nn.Linear(emb_dim, ddi_mask_H.shape[1])
        )
    ...
    def forward(self, input):
        ...
        o1= self.encoders[0](i1_seq)  # o1:(1, seq, dim*2) hi:(1,1,dim*2)
        ...
        result = torch.mul(output, bipartite_emb)
        if self.training:
            neg_pred_prob = F.sigmoid(result)
            neg_pred_prob = neg_pred_prob.t() * neg_pred_prob  # (voc_size, voc_size)
